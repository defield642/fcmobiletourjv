...
public class TournamentController {
 ...
 @PostMapping(value="/users/register",consumes="multipart/form-data") public ResponseEntity<?> register(@RequestParam("username") String username,@RequestParam("location") String location,@RequestParam(value="uid",required=false) String uid,@RequestPart(value="squadScreenshot",required=false) MultipartFile squadScreenshot){ try { return ResponseEntity.ok(registration.registerUser(username,location,uid,squadScreenshot)); } catch (Exception e) { Throwable cause=e; while(cause.getCause()!=null) cause=cause.getCause(); String message=cause.getMessage()==null?e.getClass().getSimpleName():cause.getMessage(); return ResponseEntity.status(500).body(Map.of("error","Registration failed: "+message)); } }
